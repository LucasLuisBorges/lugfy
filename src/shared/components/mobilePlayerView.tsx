...
export function MobilePlayerView() {
  ...
  return (
    <section className="fixed bottom-0 flex h-[10vh] w-full bg-gradient-to-br from-[#121212] to-[#181818] items-center justify-between px-8">
      ...
      <div className="flex gap-4 w-[30vw] items-center">
        <span className="text-white text-xs hidden sm:flex">{formatTime(currentTime)}</span>
        <input
          type="range"
          min={0}
          max={duration}
        ...
        />
        <span className="text-white text-xs hidden sm:flex">{formatTime(duration)}</span>
      </div>
      ...
      <div className="w-[30vw] flex justify-end">
        <div className="flex items-center text-white gap-2">
          ...
          <input type="range" min="0" max="1" step="0.01" value={isMuted ? 0 : volume} onChange={handleVolumeChange} className="w-[80%]" />
        </div>
      </div>
    </section>
  );
